...
drift_map <- rep("normal", n_patients)
drift_map[sample(1:n_patients, n_drift / 2)] <- "low"
drift_map[sample(setdiff(1:n_patients, which(drift_map == "low")), n_drift / 2)] <- "high"
...
generate_longitudinal <- function(ur_number, drift_type = "normal") {
  drift <- switch(drift_type, "low" = -1, "high" = +1, "normal" = 0)
  ...
  tibble(
    ...
    Collected = start_date + 
      months(0:11) + 
      days(sample(-3:3, 12, replace = TRUE)) + 
...
# Create synthetic data spaced 4 months apart - have a better way now
#generate_longitudinal <- function(ur_number, start_date = as.Date("2023-01-01")) {
#  tibble(
#    `Ur Number` = ur_number,
#    Collected = format(
#      start_date + 
#        months(seq(0, by = 4, length.out = 5)) + 
#        days(sample(0:5, 5, replace = TRUE)) + 
#        hours(sample(6:16, 5, replace = TRUE)),
#      format = "%d/%m/%Y %H:%M"
#    ),
...
## this might be easier, generic functions, can expand
## now a summary table
library(dplyr)
library(tidyr)
library(knitr)
...
coreData_with_trend %>%
  ...
  select(`Ur Number`, Collected, HGB, HGB_flag, HGB_trend) %>%
  arrange(`Ur Number`, Collected) %>%
  mutate(Collected = format(Collected, "%d-%b-%Y")) %>%
  kbl(
    caption = "Most Recent Haemoglobin Results Outside Target Range",
...
## thhis is much easier to work with I think going forwards
...
# Summarise trend proportions (last sample per patient)
...
## this might be useful on a per units analysis, hard to use with the massive glob of data
library(scales)  # for percent formatting



### new idea - would be great for indivudal units
...
ggplot(subset_data, aes(x = CollectedDate, y = HGB, group = `Ur Number`)) +
  ...
  geom_line(aes(color = highlight_status), linewidth = 0.7, alpha = 0.6) +
...
## for shiny later, this would be the approach cat(interpretation_text)
