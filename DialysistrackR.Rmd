...
set.seed(42) #Ultimate Question of Life, the Universe, and Everything
...
##lazy
 ptSubset<-coreData %>% filter(`Ur Number` == ptURN)## can clean this later - I just couldnt be bothered cleaning up the object names below
...
#    median_delta_HGB = median(delta_HGB, na.rm = TRUE),
    median_TRFSAT = median(TRFSAT, na.rm = TRUE),
    median_K = median(K, na.rm = TRUE),
    median_PTHR = median(PTHR, na.rm = TRUE),
    median_PHOS = median(PHOS, na.rm = TRUE),
    median_CAL = median(CAL, na.rm = TRUE),
    median_ALB = median(ALB, na.rm = TRUE),
    .groups = "drop"
  )
...
# Take data in specific window
dateWindow <- c("2025-01-01", "2025-12-12")
...
# Pt HB PLOT
ptCollectedDates <- c(as.Date(min(ptSubset$Collected)), as.Date(max(ptSubset$Collected))) #GLOBAL VAR
ggplot(coreData %>% filter( #keep dates within range of dateWindow dates
 ...
 #             inherit.aes = FALSE, color = "steelblue", size = 2) +
  ...
  labs(
    title = " Individual Pt Hb Saturations Over Time",
    subtitle = "",
    x = "Date", y = "Haemoglobin (g/L)"
  ) +
  scatterplotTheme
...
ggplot(coreData %>% filter( #keep dates within range of dateWindow dates
 ...
 #             inherit.aes = FALSE, color = "steelblue", size = 2) +
  ...
  labs(
    title = " Individual Pt Hb Saturations Over Time",
    subtitle = "",
    x = "Date", y = "Transferrin Saturations (%)"
  ) +
  scatterplotTheme
# TODO: ADD table listing pt values below graph
...
ggplot(coreData %>% filter( #keep dates within range of dateWindow dates
 ...
 #             inherit.aes = FALSE, color = "steelblue", size = 2) +
  ...
  scale_x_date(date_breaks = "3 months", date_labels = "%b %Y") +
  labs(
    title = " Individual Pt PTHR Over Time",
    subtitle = "",
